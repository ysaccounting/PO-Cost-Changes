...
def process_files(
    ...
    # 1b) If Purchase Details files were provided, gather their PO numbers
    #     and split merged into (kept, excluded). Source Data tab will still
    #     show everything; everything downstream uses the kept-rows DataFrame.
    ...
    if excluded_pos:
        ...
        excluded_mask = po_col.isin(excluded_pos)
        excluded_raw = merged[excluded_mask].reset_index(drop=True)
        merged_for_pipeline = merged[~excluded_mask].reset_index(drop=True)
        log.info(
            "Excluded %d PO Cost Changes rows (covering %d PO #s) found in Purchase Details",
            len(excluded_raw), excluded_raw["PO #"].nunique() if len(excluded_raw) else 0,
        )
    else:
    ...
    combined_bytes = _build_combined_workbook(
        merged, bills_df, expenses_df, all_companies, excluded_raw,
    )
    company_files = {
