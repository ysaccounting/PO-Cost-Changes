...
def count_cost_change_rows(file_list: list[tuple[bytes, str]]) -> int:
    """Count total data rows (excluding headers) across the PO Cost Changes
    uploads, using the same readers as the pipeline so the count matches what
    actually gets processed. Used by the /precheck endpoint to warn when the
    row count is an exact multiple of 500 (a sign that TicketVault pagination
    may have cut the export short)."""
    total = 0
